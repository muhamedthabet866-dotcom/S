...
# 1. دالة استخراج المتغيرات والتعريفات من الوورد
...
def generate_final_pro_syntax(paragraphs, var_map):
    # القاموس الذكي لربط الكلمات بالرموز (تأكد من توحيد الاسم هنا)
    smart_dict = {
    ...
    syntax = [
        "* Encoding: UTF-8.",
        "* =========================================================================.",
        "* UNIVERSAL MBA STATISTICAL ANALYSIS - PROFESSIONAL SYNTAX v4 (FIXED)",
    ...
    for p in paragraphs:
        ...
        if "frequency table" in p_low:
            ...
            if "categorical" in p_low or any(v in p_low for v in ["gender", "race", "region", "league", "surface", "member"]):
                ...
                target = smart_dict.get("salary", "x3") if "salary" in p_low else (smart_dict.get("age", "x9") if "age" in p_low else "x3")
            ...
            if "average" in p_low or "mean" in p_low:
                dep = smart_dict.get("children", "x8") if "children" in p_low else (smart_dict.get("salary", "x3") if "salary" in p_low else "x3")
                indep = smart_dict.get("race", "x2") if "race" in p_low else (smart_dict.get("region", "x4") if "region" in p_low else "x4")
            ...
            if "equal" in p_low and "difference" not in p_low:
                target = smart_dict.get("wins", "x7") if "wins" in p_low else "x3"
                syntax.append(f"T-TEST /TESTVAL={val} /VARIABLES={target}.")
            elif "difference" in p_low:
                if any(v in p_low for v in ["gender", "league", "surface", "member"]):
            ...
            syntax.append("* Scientific Justification: Regression measures predictors effect on dependent variable.")
...
st.title("📊 محرك الإحصاء الاحترافي (v4 - مصلح)")
...
if u_excel and u_word:
    try:
        ...
        if not paragraphs:
            ...
            st.success("✅ تم إصلاح الخطأ وتوليد السينتاكس بنجاح!")
            st.code(final_code, language='spss')
            st.download_button("تحميل السينتاكس (.sps)", final_code, "MBA_Analysis.sps")
