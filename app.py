...
def generate_exact_model_syntax(df, var_defs, questions_text):
    # مصفوفة تخزين الأسطر
    syntax = [
        "* Encoding: UTF-8.",
        "* " + "="*73 + ".",
    ...
    # إضافة القيم الثابتة (Value Labels) كما في النموذج
    ...
    # --- معالجة الأسئلة بناءً على الكلمات المفتاحية لمطابقة النموذج ---
    ...
    for q in qs:
        ...
        if "frequency table" in q_low and "categorical" in q_low:
            ...
            syntax.append("* and percentage of categorical variables like gender, race, and region.")
        ...
        # Q2-Q4: Bar Charts
            ...
            syntax.append("* and mean values (Salary/Children) across different groups.")
            if "region" in q_low and "number" in q_low:
                syntax.append("GRAPH /BAR(SIMPLE)=COUNT BY x4 /TITLE='Number of Respondents by Region'.")
            elif "salary" in q_low and "region" in q_low:
                syntax.append("GRAPH /BAR(SIMPLE)=MEAN(x3) BY x4 /TITLE='Average Salary by Region'.")
            elif "children" in q_low and "race" in q_low:
        ...
        # Q5-Q6: Pie Charts
            ...
            syntax.append("* of a whole, such as total salary distribution or gender percentage.")
            if "occupation" in q_low:
                syntax.append("GRAPH /PIE=SUM(x3) BY x11 /TITLE='Sum of Salaries by Occupation'.")
            elif "male" in q_low or "female" in q_low:
                syntax.append("GRAPH /PIE=COUNT BY x1 /TITLE='Gender Percentage'.")
            syntax.append("")

        # Q7-Q8: Recoding & Descriptives
            ...
            syntax.append("* correctly calculate Median and Mode in SPSS.")
        ...
        # Q9: Normality
            ...
            syntax.append("* Boxplots are the standard method for identifying extreme outliers.")
            syntax.append("EXAMINE VARIABLES=x3 x10 /PLOT BOXPLOT HISTOGRAM NPPLOT /STATISTICS DESCRIPTIVES.\n")

        # Q10: Grouped Analysis
        elif "each gender in each region" in q_low:
            ...
            syntax.append("* each subgroup (gender within regions) as requested.")
        ...
        # Q19-Q20: Regression
            ...
            syntax.append("REGRESSION /MISSING LISTWISE /STATISTICS COEFF OUTS R ANOVA COLLIN /CRITERIA=PIN(.05) POUT(.10) /NOORIGIN")
            syntax.append("  /DEPENDENT x5 /METHOD=ENTER x1 x2 x3 x4 x6 x7 x8 x9 x10 x11 x12.\n")

    syntax.append("EXECUTE.")
    return "\n".join(syntax)

# --- واجهة المستخدم Streamlit ---
st.title("SPSS Model Solution Generator")
uploaded_file = st.file_uploader("Upload Data Set", type=['xlsx', 'csv'])
if uploaded_file:
    df = pd.read_excel(uploaded_file) if uploaded_file.name.endswith('xlsx') else pd.read_csv(uploaded_file)
    v_in = st.text_area("Variable Definitions (Where: x1=..)", height=150)
    q_in = st.text_area("Exam Questions", height=150)
    
    if st.button("Generate Solution"):
        final_syntax = generate_exact_model_syntax(df, v_in, q_in)
