...
def universal_spss_engine(df, var_defs, questions_text):
    ...
    # 2. حساب خصائص البيانات (نستخدم 100 حالة كافتراض إذا لم يرفع ملف)
    ...
    syntax = [
        ...
        "* FIX: Pattern Matching Engine Updated",
        "* " + "="*75 + ".\n"
    ]
    ...
    # تعريف القيم (Value Labels) للمتغيرات الوصفية
    syntax.append("VALUE LABELS X4 0 'No' 1 'Yes' /X5 0 'No' 1 'Yes' /X6 1 'City 1' 2 'City 2' 3 'City 3' 4 'City 4'.")
    syntax.append("EXECUTE.\n")

    # [PHASE 2] محرك تحليل الأسئلة - تم إصلاح الخطأ هنا
    # نقوم بتقسيم النص بناءً على أرقام الأسئلة أو كلمة source
    ...
    for i, q in enumerate(questions):
        ...
        # ربط السؤال بالمتغيرات الصحيحة (x1, x2...)
        ...
        # أ. الإحصاء الوصفي والالتواء (Chapter 2)
        ...
        # ب. التكرارات والرسوم البيانية (Chapter 2)
        ...
        if any(w in q_low for w in ["compare", "difference", "between"]):
            if "city" in q_low or "more than" in q_low:
        ...
        # د. الاستكشاف والاعتدالية (Chapter 3)
        ...
        # هـ. الانحدار والارتباط (Chapter 8, 10)
...
with col2:
    q_in = st.text_area("3. الصق أي امتحان (جميع الأسئلة):", height=250)

if st.button("🚀 توليد حل الامتحان"):
    if v_in and q_in:
        df = None
        if up:
        ...
        st.subheader("✅ كود الحل الجاهز (Syntax):")
