...
def universal_spss_engine_v22(doc_upload):
    ...
    try:
        ...
        # المحاولة الثانية: استخراج النصوص الخام للملفات القديمة .doc لضمان شمولية الحل
        paragraphs = re.findall(r'[ -~]{5,}', doc_bytes.decode('ascii', errors='ignore'))

    mapping = {}
    for p in paragraphs:
        # استخراج تعريفات المتغيرات X1-X12 من ملف الوورد
    ...
    # تعريف القيم لضمان دقة تحليل البيانات الاسمية
    ...
    for p in paragraphs:
        ...
        # ربط المتغيرات بالأسئلة بناءً على الأسماء والرموز
        ...
        # --- تصحيح أمر فترات الثقة (95% و 99% - الترتيب الآمن لمنع الـ Warnings) ---
        ...
        # --- تصحيح الرسوم البيانية (منع الخطأ 701) ---
